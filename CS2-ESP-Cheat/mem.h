...
namespace mem {
	...
	public:
		...
		bool read(const Ptr& ptr,T& buffer) {
			//T buffer{};
			SIZE_T t;
			if (ReadProcessMemory(process_handle, (void*)ptr, &buffer, sizeof(T), &t))
			{
				return true;
			}
			return false;
		...
		template< typename T, typename Ptr>
		bool write(const Ptr& ptr, const T& value) {
			T v = value;
			SIZE_T write_b;
			if (WriteProcessMemory(process_handle, (void*)ptr, &v, sizeof(v), &write_b))
			{
				return true;
			}
			return false;
